...
- **색감**: 저채도 · 차분한 톤. 사이트 팔레트와 어울릴 것 —
  딥 네이비 `#0D2138` / 미드 네이비 `#143254` / 오프화이트 `#F6F9FD` / 페일 네이비 `#E7EEF7` / 블루(포인트) `#1C63C8`
  > **2026-08-27에 웜 그레이+앰버에서 네이비+화이트로 바뀌었다.** 그 이전에 만든 이미지는 웜 톤이라
  > 새 이미지와 섞이면 튄다. 교체할 때는 **기존 16장의 색보정도 같이 맞출지** 먼저 판단할 것.
- **분위기**: 실제 한국 철거·공사 현장의 담담한 기록 사진 느낌. 새벽/흐린 날의 자연광, 약간 어두운 노출.
  따뜻한 노란빛(석양·백열등)이 강한 사진은 네이비 팔레트와 부딪힌다 — 중성~푸른 기 도는 광선을 고른다.
